...
namespace ShatterToolkit {
	...
	public class ShatterTool : MonoBehaviour {
		...
		public void Shatter(Vector3 point) {
			if (!IsLastGeneration) {
				...
				for (int i = 0; i < planes.Length; i++) {
					planes[i] = new Plane(UnityEngine.Random.onUnitSphere, point);
				}
		...
		protected void CreateNewGameObjects(IList<IHull> newHulls, out GameObject[] newGameObjects) {
			...
			float newShatterResistance = 0;
			float newStuckResistance = 0;

			//set new resistances properties
			if (gameObject.GetComponent<Collectable>() != null) {
				newShatterResistance = gameObject.GetComponent<Collectable>().shatterResistance / newHulls.Count;
				newStuckResistance = gameObject.GetComponent<Collectable>().stuckResistance / newHulls.Count;

				newShatterResistance = Mathf.Round(newShatterResistance * 100f) / 100f;
				newStuckResistance = Mathf.Round(newStuckResistance * 100f) / 100f;
			}
			...
			for (int i = 0; i < newHulls.Count; i++) {
				...
				if (newGameObject.GetComponent<Collectable>() != null) {
					newGameObject.GetComponent<Collectable>().enable(newShatterResistance, newStuckResistance);
				}

				if (newGameObject.GetComponent<AudioSource>() != null) {
					Destroy(newGameObject.GetComponent<AudioSource>());
				}
