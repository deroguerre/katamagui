...
public class PlayerController : MonoBehaviour {
	...
	private void OnCollisionEnter(Collision collision) {
		...
		if (collision.gameObject.GetComponent<Collectable>() != null) {
			...
			if (collisionWeight < weight) {
				...
				//collision.gameObject.GetComponent<Cube>().isCollected = true;
				//increase size of player
				//gameObject.transform.localScale += new Vector3(increaseSizeSpeed, increaseSizeSpeed, increaseSizeSpeed);

				foreach (Transform child in transform) {
					//child.localScale -= new Vector3(0.1f, 0.1f, 0.1f);
					//child.transform.localScale += new Vector3(increaseSizeSpeed, increaseSizeSpeed, increaseSizeSpeed);
				}
	...
	void CreateMeshPiece(float extrudeSize, Vector3 pos, Material mat, int index, Vector3 faceNormal, Vector3 v1, Vector3 v2, Vector3 v3, Vector2 uv1, Vector2 uv2, Vector2 uv3, float resistance, float weight) {
		...
		//go.tag = "Collectable"; // set collectable piece
		StartCoroutine(SetCollectable(go, resistance, weight));
